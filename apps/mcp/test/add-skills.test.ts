...
import { SkillSchema } from "@skillset/shared";
import { addSkills, buildFallbackOutcome, type AddSkillsDeps } from "../src/add-skills.js";
...
function baseDeps(overrides: Partial<AddSkillsDeps> & { fetchImpl: typeof fetch; ctx: AddSkillsDeps["ctx"] }): AddSkillsDeps {
  return {
    registry: REGISTRY,
    scope: "project",
    agentId: "claude-code",
...
describe("addSkills — a normal install", () => {
  it("writes the Artifact's files into the canonical directory and links the Agent's own directory", async () => {
    await withTempRoots(async ({ cwd, homeDir }) => {
      ...
      const [outcome] = await addSkills(baseDeps({ fetchImpl, ctx: { cwd, env: {}, homeDir } }), ["code-review"]);
  ...
  it("reports which other Agents the same directory already serves", async () => {
    await withTempRoots(async ({ cwd, homeDir }) => {
      ...
      const [outcome] = await addSkills(baseDeps({ fetchImpl, ctx: { cwd, env: {}, homeDir }, agentId: "codex" }), [
        "code-review",
      ]);
  ...
  it("records an Install by requesting the Artifact with source=mcp", async () => {
    await withTempRoots(async ({ cwd, homeDir }) => {
      ...
      await addSkills(baseDeps({ fetchImpl, ctx: { cwd, env: {}, homeDir } }), ["code-review"]);
...
describe("addSkills — batching", () => {
  it("returns one outcome per Skill, in order", async () => {
    await withTempRoots(async ({ cwd, homeDir }) => {
      ...
      const outcomes = await addSkills(baseDeps({ fetchImpl, ctx: { cwd, env: {}, homeDir } }), ["skill-a", "skill-b"]);
  ...
  it("one Skill failing to resolve does not prevent the others in the batch from installing", async () => {
    await withTempRoots(async ({ cwd, homeDir }) => {
      ...
      const outcomes = await addSkills(baseDeps({ fetchImpl, ctx: { cwd, env: {}, homeDir } }), [
        "typo-name",
        "code-review",
      ]);
...
describe("addSkills — an already-installed Skill", () => {
  it("refuses when the target exists and overwrite was not requested, naming the Skill", async () => {
    await withTempRoots(async ({ cwd, homeDir }) => {
      ...
      const outcomes = await addSkills(baseDeps({ fetchImpl: secondFetch, ctx: { cwd, env: {}, homeDir } }), ["code-review"]);
  ...
  it("leaves the existing directory byte-for-byte unchanged when refused", async () => {
    await withTempRoots(async ({ cwd, homeDir }) => {
      ...
      await addSkills(baseDeps({ fetchImpl: firstFetch, ctx: { cwd, env: {}, homeDir } }), ["code-review"]);
      ...
      await addSkills(baseDeps({ fetchImpl: secondFetch, ctx: { cwd, env: {}, homeDir } }), ["code-review"]);
