...
- The shared module owns everything the CLI and the web interface must agree on with the API: the `SKILL.md` frontmatter parser, the validation schema and its limits, the Agent table, and the request and response types. This is what makes local pre-flight checks and the API's own checks the same rules rather than two drifting copies.
...
- Two naming conventions, and one place they meet. Every JSON key on the wire is snake_case, which lines wire keys up with Postgres column names. Every TypeScript type, interface, and class is camelCase. Conversion happens once, in the shared module at the API boundary, so that no route handler, query, or component does it by hand — the wire shape and the domain shape are each internally consistent, and only one module knows both.
