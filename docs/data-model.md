...
`z.infer`red from the Zod schemas in `@in-org-quicko/sqillset-shared`, so a Drizzle row, a wire payload,
...
`@in-org-quicko/sqillset-shared` — the same treatment `integrations.provider` gets against
...
union in `@in-org-quicko/sqillset-shared` (ADR-0026). For `skill`, this is the four optional Agent Skills
...
a Kind's Artifact through the API) or `cli` (`sqillset add`, ticket 09). Downloading a Skill's
...
`@in-org-quicko/sqillset-shared`, checked in the service. Since ADR-0025, `provider` carries no uniqueness
