...
> this ADR withdrew is now the design — `install` writes to `.agents/skills` and symlinks each
...
`skillset install` implements ADR-0006's full five verified Agents (`claude-code`, `codex`,
...
`install` now writes the Skill into exactly one directory: the one the chosen Agent reads.
...
`install` reports every Agent that reads the directory it wrote to — via
...
Installing for several Agents means running `install` once per Agent. That is a real cost, and
